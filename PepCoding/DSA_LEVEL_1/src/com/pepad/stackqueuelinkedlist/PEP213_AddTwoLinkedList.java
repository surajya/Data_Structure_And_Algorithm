...
public class PEP213_AddTwoLinkedList {
	...
	static PEP151_LinkedList addLinkedList;
	static int carry = 0;
	...
	static void initLinkedList() {
		...
		addLinkedList = new PEP151_LinkedList();
		addLinkedList.head = null;
		addLinkedList.tail = null;
		addLinkedList.sz = 0;
	}
	...
	public static void foldLLRecursive(PEP150_Node head, int floor) {
		if (head == null) {
			return;
		}
		foldLLRecursive(head.next, floor + 1);
		if (floor > linkedlist.sz / 2) {
			head.next = null;
		}

		if (floor > (linkedlist.sz / 2) + 1) {
			PEP150_Node testNode = testHead.next;
			testHead.next = head;
			head.next = testNode;
			testHead = testNode;
		}
	...
	private static void addTwoLinkedList(PEP150_Node node1, PEP150_Node node2, int sizeGap, int stage) {
		if (node1 == null) {
			return;
		}

		if (sizeGap > 0) {
			addTwoLinkedList(node1.next, node2, sizeGap - 1, stage);
		} else {
			addTwoLinkedList(node1.next, node2.next, sizeGap, stage);
		}
		int sumOfDigit = 0, sumData = 0;
		if (sizeGap == 0) {
			sumOfDigit = node1.data + node2.data + carry;
			if (sumOfDigit >= 10) {
				sumData = sumOfDigit % 10;
				carry = 1;
			} else {
				sumData = sumOfDigit;
				carry = 0;
		...
		PEP150_Node addDataNode = new PEP150_Node();
		addDataNode.data = sumData;
		if (addLinkedList.head == null) {
			addLinkedList.head = addDataNode;
			addLinkedList.tail = addDataNode;
		} else {
			addDataNode.next = addLinkedList.head;
			addLinkedList.head = addDataNode;
		}
		if (sizeGap == stage && carry == 1) {
			PEP150_Node addDataNode1 = new PEP150_Node();
			addDataNode1.data = 1;
			addDataNode1.next = addLinkedList.head;
			addLinkedList.head = addDataNode1;
		}
	}
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			switch (index) {
				...
				case 14:
					testHead = linkedlist.head;
					foldLLRecursive(linkedlist.head, 1);
					displayLinkedList(linkedlist);
					break;

				case 15:
					initBothLinkedList();
					int sizeGap = linkedlist1.sz - linkedlist2.sz ;
					if (sizeGap >= 0) {
						addTwoLinkedList(linkedlist1.head, linkedlist2.head, sizeGap, sizeGap);
					} else {
						addTwoLinkedList(linkedlist2.head, linkedlist1.head, sizeGap * -1, sizeGap * -1);
					}
					displayLinkedList(addLinkedList);
					break;
