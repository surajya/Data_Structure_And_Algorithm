...
public class PEP334_TransformBackFromLeftCloneTree {
	...
	public static void transBackToLeftCloneTree(Node node) {
		if (node == null) {
			return;
		}
		transBackToLeftCloneTree(node.leftNode);
		if (node.leftNode != null && node.leftNode.data == node.data) {
			node.leftNode = node.leftNode.leftNode;
		}
		transBackToLeftCloneTree(node.rightNode);
	}

	public static void main(String[] args) {
		int[] arr =
				{50, 50, 25, 25, 12, 12, -1, -1, -1, -1, 37, 37, 30, 30, -1, -1, -1, -1, -1, -1, 75, 75, 62, 62, -1, -1,
						70, 70, -1, -1, -1, -1, 87, 87, -1, -1, -1};
		Node root = constructBT(arr);
		//display(root);
		transBackToLeftCloneTree(root);
		display(root);
	}
}
