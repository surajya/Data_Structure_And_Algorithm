...
public class PEP261_EEFLinearizeGenericTree_REVISE {
	...
	static void lineariseGT(Node root) {
		for (Node childNode : root.children) {
			lineariseGT(childNode);
		}

		while (root.children.size() > 1) {
			Node remove = root.children.remove(root.children.size() - 1);
			Node node = root.children.get(root.children.size() - 1);
			Node lsNode = getLastNode(node);
			lsNode.children.add(remove);
		}
	...
	static Node getLastNode(Node node) {
		...
		return getLastNode(node.children.get(0));
	}

	static Node lineariseGTEff(Node root) {
		if (root.children.isEmpty()) {
			return root;
		}
		Node lsnNode = lineariseGTEff(root.children.get(root.children.size() - 1));
		while (root.children.size() > 1) {
			Node lsNode = root.children.remove(root.children.size() - 1);
			Node sclNode = lineariseGTEff(root.children.get(root.children.size() - 1));
			sclNode.children.add(lsNode);
		}
	...
	public static void main(String[] args) {
		...
		//		System.out.println("\nLinearise Generic Tree : ");
		//		lineariseGT(root);
		//		displayGT(root);

		System.out.println("\nLinearise Generic Tree efficientway: ");
		lineariseGTEff(root);
		displayGT(root);
