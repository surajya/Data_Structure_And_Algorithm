...
public class PEP284_TreeInSymmetric{
	...
	private static Boolean checkTreeSymmetric(Node root2) {
		// TODO Auto-generated method stub
		int i = 0, j = root2.children.size() - 1;
		while (i < j) {
			if (root2.children.get(i).children.size() != root2.children.get(j).children.size()) {
				return false;
			}
			i++;
			j--;
		...
		for (Node childNode : root2.children) {
			if (!checkTreeSymmetric(childNode)) {
				return false;
			}
		}
		return true;
	...
	public static void main(String[] args) {
		...
		int[] eularGT =
				{10, 20, 50, -1, 60, -1, -1, 30, 70, -1, 80, 110, -1, 120, 125, -1, -1, -1, 90, -1, -1, 40, 95, -1, 100,
						-1, -1,
						-1};
		...
		System.out.println("Tree are symmetric: " + checkTreeSymmetric(root));
